...
export const Layout = ({}: ClockProps) => {
  ...
  const [timezone, setTimezone] = useLocalStorageState<string>('nf_timezone', '');
  ...
  const getTimeInTimezone = () => {
    ...
    return new Date(currentTime.toLocaleString('en-US', { timeZone: effectiveTz as string }));
  };
  ...
  return (
    <div className="min-h-screen bg-background relative overflow-hidden">
      ...
      <div className="absolute bottom-20 right-20 w-96 h-96 bg-secondary/15 rounded-full blur-3xl animate-float" style={{ animationDelay: '1s' }}></div>
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        ...
        >
          <div className="grid grid-cols-3 items-center mb-4">
            ...
            <h1 className="col-start-2 justify-self-center text-6xl font-bold bg-gradient-primary bg-clip-text text-transparent text-glow-primary">
              Nightfall
            </h1>
            <div className="justify-self-end"><ThemeSwitcher /></div>
          </div>
        ...
        <motion.div 
          ...
          className="w-full max-w-none mx-auto mb-8 px-4"
        >
          <ClockFace 
        ...
        >
          <WeatherWidget onWeatherUpdate={(w) => { setWeather(w); }} />
          <FactOfTheDay />
        </motion.div>
        ...
        <motion.div 
          ...
          className="mt-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 max-w-6xl mx-auto"
        >
          {[
            ...
            <div key={city.tz} className="card-gothic p-4 text-center hover:bg-card/80 transition-colors cursor-pointer group">
              <h3 className="text-lg font-semibold text-primary group-hover:text-white transition-colors">{city.name}</h3>
              <p className="text-2xl font-mono text-foreground group-hover:text-white transition-colors">
